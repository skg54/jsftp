...
Ftp.prototype.emitProgressBuffer = function(data) {
  this.emit('progress', {
    filename: data.filename,
    action: data.action,
    total: data.totalSize || 0,
    transferred: data.transferred //data.socket[data.action === 'get' ? 'bytesRead' : 'bytesWritten']
  });
};
...
Ftp.prototype.put = function(from, to, callback) {
  ...
  if (from instanceof Buffer) {
    ...
    console.log('JSFTP from instanceof Buffer');
    ...
    this.getPutSocket(to, function(err, socket) 
    {
      ...
      if (!err) 
      {
        console.log('JSFTP getPutSocket socket NO ERROR');
        ...
        socket.on('error', function (err) {
          console.log('JSFTP $$$$$$$$$$$$ socket.on error == ', err);
        });
        ...
        socket.on("drain", function () {
          ...
          if(done) 
          {
            ...
            self.socketIO.emit('progress', progressObj);

            console.log('JSFtp socket.end() -- self.socketIO.emit EVENT == ' + JSON.stringify(progressObj));
          ...
          else
          {
            ...
            console.log('JSFtp self.socketIO.emit EVENT == ' + JSON.stringify(progressObj));
          }
        })
      }
      else
      {
        console.log('JSFTP getPutSocket err == ', err);
      }
    }, callback);
    ...
    console.log('JSFTP from instanceof string');
    ...
    console.log('JSFTP from is a readable stream');
